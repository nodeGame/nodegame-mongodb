...
MongoLayer.prototype.connect = function(callback) {
    ...
    this.db.open(function(err, db) {
        if (err !== null) {
            that.node.err('MongoLayer.connect: error opening MongoDB ' +
                          'connection: ' + err);
            return;
        }
        ...
        db.collection(that.collectionName, function(err, collection) {
            if (err !== null) {
                that.node.err("MongoLayer.connect: error opening collection '" +
...
MongoLayer.prototype.store = function(data) {
    ...
    if ('object' !== typeof data) {
        this.node.err('MongoLayer.store: data must be object.');
        return false;
    }

    // add timestamp:
    if (data.hasOwnProperty('time')) {
        this.node.err("MongoLayer.store: data object cannot have 'time' field.");
        return false;
    }
    ...
    if (!this.activeCollection) {
        this.node.err('MongoLayer.store: no active connection.');
        return false;
    }
